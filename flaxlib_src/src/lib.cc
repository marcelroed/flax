...
namespace flaxlib
{
  ...
  // fingerprint
  //---------------------------------------------------------------
  std::tuple<std::vector<std::tuple<nb::object, nb::object>>, nb::object> _key_values_metadata(
      PythonContext &ctx,
      nb::object &node,
      nb::object &node_impl)
  {
    ...
    {
      auto nodes = nb::cast<std::map<std::string, nb::object>>(node.attr("__dict__"));
      nodes.erase("_object__state");
      std::vector<std::tuple<nb::object, nb::object>> nodes_vec;
      for (const auto &[key, value] : nodes)
      {
        nodes_vec.push_back({nb::str(key.c_str()), value});
      }
      auto metadata = nb::make_tuple(node.type(), node.attr("_object__state").attr("_initializing"));
      return {nodes_vec, metadata};
    }
    else
    {
      ...
      return {nb::cast<std::vector<std::tuple<nb::object, nb::object>>>(values), metadata};
    }
  }
